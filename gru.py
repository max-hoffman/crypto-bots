...
def GRUCell(x_in, input_size, s_start, hidden_size):
    with tf.name_scope('gru-cell'):
        ...
        if tf.rank(x_in) == 1:
            z = tf.sigmoid(tf.matmul(tf.expand_dims(x_in,0), u_xz) + tf.matmul(s_start, w_sz) + b_z)
            r = tf.sigmoid(tf.matmul(tf.expand_dims(x_in,0), u_xr) + tf.matmul(s_start, w_sr) + b_r)
            h = tf.tanh(tf.matmul(tf.expand_dims(x_in,0), u_xh) + tf.matmul(tf.multiply(s_start, r), w_sh) + b_h)
        else:
